...
MTOM = 270; % tonnes
no_landings = 50; % number of landings per year
ICAO_taxi_configuration = 'D'; % ICAO taxi configuration
no_days_landing = 50; % number of days the aircraft is parked at the airport per year
...
total_flight_ours = 2000; % total flight hours per year
% Crew cost
...
total_landing_fees = landing_fee_per_aircraft * no_landings; % $ per year
...
total_parking_fees = parking_fee_per_day * no_days_landing; % $ per year
...
total_fuel_cost = total_fuel_price_per_litre * fuel_consumption_per_flight; % $ per year
...
total_maintenance_cost = maintenance_fixed_cost + maintenance_variable_cost*total_flight_ours; % $ per year
